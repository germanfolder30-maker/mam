...
local instantConnection = nil   -- для удержания E
...
-- ===================== INSTANT MINE (удержание E) =====================
local function setInstantMine(enabled)
    if enabled then
        -- Отправляем нажатие E (KeyDown) – клавиша будет удерживаться
        VirtualInput:SendKeyEvent(true, Enum.KeyCode.E, false, game)
        -- При необходимости можно добавить дополнительный контроль, но одиночный вызов надёжен
    else
        -- Отпускаем E (KeyUp)
...
-- Дополнительная защита: при смене персонажа восстанавливаем состояние
player.CharacterAdded:Connect(function()
    if states.instant then
        task.wait(0.3)  -- даём время на загрузку
        VirtualInput:SendKeyEvent(true, Enum.KeyCode.E, false, game)
    end
end)
...
-- 2) No Fall (заглушка – можно заменить реальным кодом при необходимости)
...
-- 4) Instant Mine (реализация через удержание E)
addToggleButton("⛏️", "Instant Mine", "instant", setInstantMine)

-- 5) Дополнительная кнопка (можно заменить на что-то полезное)
...
print("✓ Меню готово. Instant Mine удерживает E при включении.")
