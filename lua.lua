--[[
    БЕЛОЕ МЕНЮ ДЛЯ MINE A MOUNTAIN (x2 Speed + Большой урон кирки)
]]
...
local states = {
    ...
    nofall = false,
    backpack = false,
    bigDamage = false,   -- переименовали
}

local speedConnection = nil
local originalDamage = nil   -- для сохранения оригинального урона кирки
...
local function setBigDamage(enabled)
    ...
    -- Пытаемся найти свойство, отвечающее за урон/скорость добычи
    -- Обычно это "Damage", "MiningPower", "Power" или "MiningSpeed"
    local possibleProps = {"Damage", "MiningPower", "Power", "MiningSpeed", "PickaxePower"}
    local foundProp = nil
    for _, propName in ipairs(possibleProps) do
        if tool:FindFirstChild(propName) and tool[propName]:IsA("NumberValue") then
            foundProp = tool:FindFirstChild(propName)
            break
        end
    end

    if not foundProp then
        -- Если не нашли NumberValue, попробуем найти атрибут
        for _, attrName in ipairs(possibleProps) do
            if tool:GetAttribute(attrName) ~= nil then
                foundProp = {Name = attrName, Value = tool:GetAttribute(attrName), IsAttribute = true}
                break
            end
        end
    end
    ...
    if not foundProp then
        warn("❌ Не удалось найти свойство урона у кирки. Возможно, оно называется иначе.")
        return
    end

    if enabled then
        -- Сохраняем оригинальное значение
        if foundProp.IsAttribute then
            originalDamage = {Value = foundProp.Value, IsAttribute = true, Name = foundProp.Name}
        else
            originalDamage = {Value = foundProp.Value, IsAttribute = false, Object = foundProp}
        end
        -- Устанавливаем огромный урон
        local newValue = 99999
        if foundProp.IsAttribute then
            tool:SetAttribute(foundProp.Name, newValue)
        else
            foundProp.Value = newValue
        end
        print("✅ Урон кирки увеличен до " .. newValue)
    else
        -- Восстанавливаем оригинальное значение
        if originalDamage then
            if originalDamage.IsAttribute then
                tool:SetAttribute(originalDamage.Name, originalDamage.Value)
            else
                originalDamage.Object.Value = originalDamage.Value
            end
            originalDamage = nil
            print("✅ Урон кирки восстановлен.")
        end
    end
end

-- Также при смене кирки или персонажа нужно переприменить, если включено
player.CharacterAdded:Connect(function(char)
...
addToggleButton("💥", "Большой урон", "bigDamage", setBigDamage)   -- новая кнопка
addToggleButton("❓", "???", "???", function(on) print("??? : " .. (on and "ON" or "OFF")) end)
...
print("✓ Меню готово. Включи 'Большой урон', возьми кирку и зажми E — добыча будет мгновенной.")
