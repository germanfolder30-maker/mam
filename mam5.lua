--[[
    CUSTOM MENU GUI – Белая тема + горизонтальное сворачивание (исправлено)
    - Убран тёмный выступ за рамку (тень)
    - В свёрнутом виде остаётся только аккуратная полоса с кнопкой
    - Иконка-ромбик заменена на лаконичную точку, которая скрывается при сворачивании
]]
...
-- ===================== ЦВЕТА (светлая тема) =====================
local COLORS = {
    Background   = Color3.fromRGB(242, 242, 247),   -- светло-серый
    Panel        = Color3.fromRGB(235, 235, 240),
    Stroke       = Color3.fromRGB(255, 255, 255),   -- белая рамка
...
local WINDOW_SIZE_COLLAPSED = UDim2.fromOffset(300, 60)   -- узкая полоска
...
local mainFrame = new("Frame", {
    ...
    BorderSizePixel = 0,
    ClipsDescendants = true,     -- обрезает всё за краями
    Parent = screenGui,
})
addCorner(mainFrame, 20)
addStroke(mainFrame, COLORS.Stroke, 1.8, 0)   -- белая обводка
...
-- Примеры пунктов (будущие функции)
addMenuItem("🏠", "Главная")
...
local function setCollapsed(state)
    ...
    -- Скрываем элементы, не нужные в свёрнутом виде
...
print("✓ Белое меню готово. Перетаскивай, сворачивай – теперь без лишних теней и ромбиков.")
