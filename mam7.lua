--[[
    БЕЛОЕ МЕНЮ ДЛЯ MINE A MOUNTAIN (x2 Speed – принудительное обновление)
...
-- Соединения для принудительного обновления
...
-- Включаем/выключаем принудительное обновление
...
-- При возрождении перезапускаем, если активно
player.CharacterAdded:Connect(function(char)
    if states.speed then
        -- Дадим немного времени на загрузку персонажа
        task.wait(0.2)
        forceSpeed()
        -- Переподключаем цикл на нового персонажа
        if speedConnection then speedConnection:Disconnect() end
        speedConnection = RunService.RenderStepped:Connect(forceSpeed)
    end
end)
...
-- 1) x2 Speed (работает принудительно)
...
print("✓ Меню готово. x2 Speed теперь принудительно обновляется каждый кадр.")
