--[[
    ...
    Использует RemoteEvent: takeOutCrystall
]]
...
-- ========== INSTANT MINE (через takeOutCrystall) ==========
local REMOTE_NAME = "takeOutCrystall"   -- точное имя события
local EXTRA_ARG = nil                   -- если нужен второй аргумент, укажи: true, 1, "Start" и т.д.

local mineRemote = nil

-- Поиск RemoteEvent (если не найден, выведет ошибку)
local function findMineEvent()
...
-- Цикл Instant Mine (очень частый)
local function instantLoopFunc()
    ...
    while states.instant do
        ...
        task.wait(0.02)  -- очень часто, чтобы шкала падала моментально
    end
end
...
local function setInstantMine(enabled)
    if enabled then
        if not mineRemote then
            mineRemote = findMineEvent()
            if not mineRemote then
                warn("❌ Не найден RemoteEvent с именем " .. REMOTE_NAME .. ". Проверь имя или обнови скрипт.")
                return
            else
...
print("✓ Меню готово. Instant Mine использует RemoteEvent 'takeOutCrystall'.")
