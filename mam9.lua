--[[
    БЕЛОЕ МЕНЮ ДЛЯ MINE A MOUNTAIN
    x2 Speed (принудительно) + Instant Mine (тап E без удержания)
]]
...
-- ===================== INSTANT MINE (ПЕРЕХВАТ E) =====================
UserInputService.InputBegan:Connect(function(input, gameProcessed)
    if gameProcessed then return end  -- не вмешиваемся в GUI
    if input.KeyCode == Enum.KeyCode.E then
        ...
        -- Ищем все ProximityPrompt, которые сейчас активны и рядом с игроком
        for _, obj in ipairs(workspace:GetDescendants()) do
            if obj:IsA("ProximityPrompt") and obj.Enabled then
                -- Проверяем расстояние (MaxActivationDistance)
                local dist = (obj.Parent and obj.Parent:IsA("BasePart") and (obj.Parent.Position - root.Position).Magnitude) or 999
                if dist <= obj.MaxActivationDistance then
                    -- Мгновенно начинаем и завершаем ввод
                    pcall(function()
                        obj:InputHoldBegin()
                        task.wait(0.02)
                        obj:InputHoldEnd()
...
-- ===================== ФУНКЦИИ МЕНЮ =====================
local states = { speed = false, nofall = false, backpack = false, instant = true }

-- x2 Speed (принудительное обновление)
...
addToggleButton("⛏️", "Instant Mine", "instant", function(on) print("Instant Mine всегда активна!") end)
addToggleButton("❓", "???", "???", function(on) print("???:", on) end)
...
print("✓ Готово: x2 Speed, Instant Mine (тап E мгновенно собирает кристаллы).")
