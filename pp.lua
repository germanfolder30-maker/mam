--[[
    ...
    Исправленная версия (только через кирку).
]]
...
local savedValues = {}   -- для сохранения оригинальных значений
...
-- ========== БОЛЬШОЙ УРОН (только через кирку) ==========
local function getPickaxe()
    local char = player.Character
    if not char then return nil end
    -- Ищем Tool в руках (это и есть кирка)
    local tool = char:FindFirstChildOfClass("Tool")
    if tool then
        return tool
    end
    -- Если не нашли, пробуем перебрать всех детей (на всякий случай)
...
local function findStats(tool)
    local stats = {}
    if not tool then return stats end
    -- NumberValue
...
local function setBigDamage(enabled)
    ...
    if #stats == 0 then
        warn("❌ У этой кирки не найдено характеристик урона (NumberValue или атрибуты). Попробуй другую кирку.")
        return
    end
...
print("✅ Меню загружено. Включи 'Большой урон', возьми кирку и смотри в консоль.")
