...
-- ========== INSTANT MINE (МГНОВЕННОЕ НАЧАЛО ДОБЫЧИ) ==========
local mineRemote = nil
local REMOTE_NAME = nil  -- можно задать вручную, например "MineCrystal"
local EXTRA_ARG = nil    -- если нужен второй аргумент, например true или "Start"

-- Поиск RemoteEvent
local function findMineEvents()
    -- Если задано имя вручную, ищем его
    if REMOTE_NAME then
        local found = game:GetDescendants():FindFirstChild(REMOTE_NAME, true)
        if found and found:IsA("RemoteEvent") then return found end
    end
    -- Иначе ищем по ключевым словам
    local keywords = {"Mine", "Crystal", "Break", "Harvest", "Start", "Dig", "Destroy"}
...
-- Функция вызова добычи
local function triggerMine(target)
    if not mineRemote then return end
    -- Пробуем вызвать с объектом
    if EXTRA_ARG then
        mineRemote:FireServer(target, EXTRA_ARG)
    else
        mineRemote:FireServer(target)
    end
    -- Если не сработало, можно попробовать ещё варианты:
    -- mineRemote:FireServer(target, 99999)
    -- mineRemote:FireServer(target, true)
    -- Но лучше сначала задать EXTRA_ARG вручную
end

-- Основной цикл (очень быстрый)
local function instantLoopFunc()
    local mouse = player:GetMouse()
    while states.instant do
        local target = mouse.Target
        if target and (target.Name:find("Crystal") or target.Name:find("Rock") or target.Name:find("Ore")) then
            triggerMine(target)
        else
            -- Ищем ближайший кристалл
            local char = player.Character
            if char and char:FindFirstChild("HumanoidRootPart") then
                ...
                for _, obj in pairs(workspace:GetDescendants()) do
                    if obj:IsA("BasePart") and (obj.Name:find("Crystal") or obj.Name:find("Rock") or obj.Name:find("Ore")) and obj:IsA("Part") then
        ...
        task.wait(0.02)  -- часто, чтобы удержание считалось завершённым
    end
end
...
local function setInstantMine(enabled)
    if enabled then
        if not mineRemote then
            mineRemote = findMineEvents()
            if not mineRemote then
                warn("❌ Не найден RemoteEvent. Укажи имя в REMOTE_NAME.")
                return
            else
                print("✓ Найден RemoteEvent: " .. mineRemote.Name)
            end
        end
        if not instantLoop then
            states.instant = true
            instantLoop = task.spawn(instantLoopFunc)
        end
    else
...
print("✓ Меню готово. Instant Mine вызывает событие добычи без ожидания.")
